...
- **System Improvement Report**: Console output after every Job 2 run showing CRITICAL/HIGH/MEDIUM/LOW findings with full teacher background details (ID, max load, current periods used, complete prescribed course load from Template 6, feasibility assessment — MOVE vs ADD), blocking chain analysis, teacher bottlenecks, period hotspots, and cross-run conflict delta tracking
