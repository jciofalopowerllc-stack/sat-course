...
| B17 | 08.08.26 | `min()` empty sequence crash in `full_reseat()`/`full_reseat_fast()` when ALL sections of a course are unplaced | Bug B1 filter removes unplaced SIDs from `sec_by_code`, but `sec_by_code` is `defaultdict(list)` — accessing a deleted key re-creates an empty list. `min()` on empty list → `ValueError`. Latent bug exposed when 6 sections unplaced (including ALL sections of 472 Web Design and 726 Business Concepts). | Added `if not _fr_opts: continue` guard before every `min()` call on `sec_by_code[cid]` — 6 locations total (3 in `full_reseat`, 3 in `full_reseat_fast`). | `schedule_engine_v3.py` `full_reseat()` and `full_reseat_fast()` |
| B18 | 08.08.26 | Phase D-2 combined consecutive-6 violation — Steps 1+2 pass independently but combined result violates constraint | Phase D-2 validates displacement (`_can_move_section`) and placement (`_can_place_unplaced`) independently. Each passes with 5 occupied periods. But combined: displaced section moves to new period AND unplaced section fills freed period — teacher can end up with 6 periods in a semester with free at A or G. Example: Corcoran S1 displaced 757 D→E (5 periods OK) + placed 727 in D (5 periods OK) = A,B,C,D,E,F occupied (6 consecutive, free=G). | Added Step 3 between Step 2 and SUCCESS block: temporarily applies BOTH changes, checks each semester for 6 occupied periods with free at A or G, reverts both. If violation found, increments `_d2_skip_move` and `continue`s to next candidate. | `schedule_engine_v3.py` Phase D-2 ~line 6828 |
| B19 | 08.08.26 | Engine ignores T7 Column 7 section numbers — auto-generates instead | Line 1768-1769 used `_section_counter` to auto-generate section numbers (1,2,3…) based on T7 row order, never reading T7 Column 7 (`Section #`). When T7 rows for a course are ordered differently than section numbers (e.g., 631 rows: Sec#3,4,5,6,1,2), the engine renumbers them 1,2,3,4,5,6, so output section numbers don't match T7. Affected 76 sections across 14 courses. No prescribed term violations resulted (halves were set correctly from prescribed_term), but output was confusing when compared to T7. | Read T7 Column 7 value and use it as section number. Fallback to auto-counter only if T7 Col 7 is blank. | `schedule_engine_v3.py` ~line 1768 |
| B20 | 08.08.26 | `_teacher_aware_ec_rebalance()` counted raw sections per semester instead of unique period slots — co-schedule groups inflated count | Co-schedule groups (e.g., McConnell's Robotics Block: 590×2 + 591×2 = 4 sections sharing 1 period) were counted as 4 S1 sections. Function saw S1=8 (actually 5 unique periods), thought over cap(6), flipped 2 EC sections S1→S2. Created S2=6 with no room for 570 Introduction to Robotics. Same bug affected Dennehy (AP Art Block, Studio Art Block). | Changed from raw `sum(1 for sid in rb_all_sids if 'S1' in sections[sid]['halves'])` to co-schedule-aware counting: track `_rb_cg_s1`/`_rb_cg_s2` sets of co-group indices, increment count only once per co-group. Result: McConnell S1=5(correct), S2=4(correct), 0 EC flips needed. | `schedule_engine_v3.py` `_teacher_aware_ec_rebalance()` ~line 3161 |
...
- **sec_by_code empty-list guard**: `sec_by_code` is `defaultdict(list)`. After the Bug B1 filter deletes courses with all sections unplaced, accessing `sec_by_code[cid]` re-creates an empty list instead of raising `KeyError`. Every `min(sec_by_code[cid], ...)` call MUST be preceded by `if not _opts: continue`. The `if cid not in sec_by_code` check alone is insufficient with defaultdict.
...
- **Phase D-2 combined consecutive-6 check**: After Steps 1 and 2 pass independently, Step 3 MUST simulate BOTH changes simultaneously (displaced→new period + unplaced→freed period) and check for consecutive-6 violations in EVERY semester. Two moves that each leave 5 periods occupied can combine to 6 periods occupied. Always revert both changes after the check, and call `_invalidate_occ_cache()` after both the simulation and the revert.
- **T7 section numbers are authoritative**: The section number for each section comes from T7 Column 7 (`Section #`), NOT from auto-incrementing a counter. Auto-generated numbers diverge from T7 when rows are not sorted by section number (e.g., 631 rows ordered Sec#3,4,5,6,1,2 → auto-counter produces 1,2,3,4,5,6). Always read Column 7; auto-counter is fallback only when Column 7 is blank.
- **Prescribed term is sacred**: The engine MUST place every section in its prescribed semester (T7 Column 8). S1→S1, S2→S2, FY→FY, EC→engine choice of S1/S2. No exceptions. No code path may change a section's `halves` away from its prescribed term unless `prescribed_term == 'EC'`.
- **Co-schedule counting in load checks**: Any code that counts a teacher's sections per semester for load cap comparison MUST count unique PERIOD SLOTS, not raw section count. Co-schedule groups (multiple sections sharing one period) inflate raw counts. Use a set of co-group indices (`code_to_cogroup`) — increment count only once per co-group. Raw counting caused B20: McConnell's 4 co-scheduled sections (1 period) counted as 4 → false over-cap → unnecessary EC flips → downstream UNPLACED section.
